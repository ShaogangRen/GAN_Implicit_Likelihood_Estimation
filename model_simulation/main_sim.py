...
class Var_Net(nn.Module):
    def __init__(self, K=20, output_dim=2, use_cuda=True):
        ...
        self.W = nn.Parameter(torch.rand(self.K, self.output_dim).cuda() if self.use_cuda else
                              torch.rand(self.K, self.output_dim), requires_grad=True)
...
## input_dim for D and IG is X. input_dim for G is Z
## output_dim of D is 1 or 0, output_dim of IG is Z, output_dim of G is X.
class InvGAN(object):
    ...
    def __init__(self, opt, train_hist,dataloader = None):
        ...
        fname = "data_2_6_train.p"
        pdata = pickle.load(open(fname, "rb"))
        self.xdata = np.array(pdata['X'])
        self.llkdata = np.array(pdata['likelihood'])
        self.Batch_Max = self.xdata.shape[0]//self.opt.batch_size

        fname = "data_2_6_train.p"
        pdata_test = pickle.load(open(fname, "rb"))
        self.xdata_test = np.array(pdata_test['X'])
        self.llkdata_test = np.array(pdata_test['likelihood'])
    ...
    def get_batch_data(self, it):
        ...
        return self.xdata[self.opt.batch_size * idx:self.opt.batch_size * (idx + 1), :], \
               self.llkdata[self.opt.batch_size * idx:self.opt.batch_size * (idx + 1), :]
    ...
    def get_batch_data_test(self, it):
        ...
        return self.xdata_test[self.opt.batch_size * idx:self.opt.batch_size * (idx + 1), :], \
               self.llkdata_test[self.opt.batch_size * idx:self.opt.batch_size * (idx + 1), :]
    ...
    def train(self):
        ...
        fcounter = 1
        #fw = open('out/workfile_{}.txt'.format(fcounter), 'w')
        ...
        for it in range(self.opt.iter_gan):
            ...
            if it >= 4000:
                ...
                if it % 500 == 0:
                    findex_test = 0
                    for i_test in range(self.Batch_Max_test):
                        x_, llk_grnt = self.get_batch_data_test(findex_test)
                        ...
                        if i_test == 0:
                            ...
                            llk_grnd_all = llk_grnt
                        else:
                            llk_test_all = np.concatenate((llk_test_all, llk_est), axis=0)
                            llk_grnd_all = np.concatenate((llk_grnd_all, llk_grnt), axis=0)
                    print('Ite ={}  test llk_mean = {}\n'.format((it + 1), np.mean(llk_test_all)))
                    filename = out_dir + "/likelihood_2_6_NoZZ_{}.mat".format(it)
                    savemat(filename, {'llk_est': llk_test_all, 'llk_grdtrue': llk_grnd_all})
